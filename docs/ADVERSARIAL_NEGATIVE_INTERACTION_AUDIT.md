...
## Runtime Watchdog 0.3 decision

**Watchdog 0.3 is intentionally cancelled.**

After the adversarial audit closed the negative frontier, a new permanent runtime watchdog would have to duplicate the same semantic knowledge already present in the accepted task/lifecycle/navigation model: task ownership, lifecycle ownership, ancestor projection, same-visit suppression, event-only stages, exact authored IDs, and equivalent completion branches.

That would create a second classifier with its own drift and disagreement failure modes. Watchdog 0.2 already demonstrated this risk: it reached `UNKNOWN=0` while still containing six wrong semantic-negative decisions.

The accepted ongoing safety strategy is therefore:

- keep the exhaustive static validator as the structural regression gate;
- keep Watchdog 0.2 frozen as historical research provenance, not a semantic oracle;
- do **not** build or maintain Watchdog 0.3;
- when a concrete runtime contradiction appears during play (missing marker, extra marker, or unexpected transition), investigate that state with a narrow read-only diagnostic rather than a permanent parallel classifier.

This is an application of the least-sufficient-mechanism rule, not a reduction in required evidence.
...
- **Point 1 — adversarial audit of negative interaction cases: complete.**
- **Point 2 — Runtime Watchdog 0.3: intentionally cancelled as redundant after Point 1.**
- **Point 3 — audit residual `VerifiedCompletionReminderRules`: next.**
- **Point 4 — final coverage audit: pending after Point 3.**

The next technical question is whether the remaining verified completion supplement can be reduced or eliminated by the current generic task/dialogue architecture without weakening evidence or reintroducing broad provenance parsing.

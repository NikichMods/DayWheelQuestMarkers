...
namespace CalendarQuestsPins
{
    ...
    internal sealed class WeekdayInteractionRuleCache
    {
        ...
        private void ParseGraph(TargetRules target, string serialized, Dictionary<string, object> knownNpcMap)
        {
            ...
            // Keep the standalone topic layer aligned with the task rules that production
            // actually compiled. Anchor-only bookkeeping can lag behind when verified owner-task
            // topology expands, which must never leave a task-owned answer as a one-shot reminder.
        ...
        private static void AddCompiledTaskRuleAnswerIds(HashSet<string> ids, TargetRules target)
        {
            if (ids == null || target == null) return;
            foreach (var pair in target.OwnerTaskRules)
                AddSupportedRuleAnswerIds(ids, pair.Value);
            for (var i = 0; i < target.CrossTasks.Count; i++)
                if (target.CrossTasks[i] != null) AddSupportedRuleAnswerIds(ids, target.CrossTasks[i].Rules);
        }

        private static void AddSupportedRuleAnswerIds(HashSet<string> ids, List<RuleVariant> variants)
        {
            if (ids == null || variants == null) return;
            for (var i = 0; i < variants.Count; i++)
            {
                var variant = variants[i];
                if (variant == null || variant.Unsupported || string.IsNullOrEmpty(variant.AnswerId)) continue;
                ids.Add(variant.AnswerId);
            }
        }
